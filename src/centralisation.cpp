...
void Graphe::centralisation()
{
    ...
    allOctagons.clear();      // Clear the list of octagons before centralisation
    ...
    for (int i = 0; i < pointList.size(); ++i)
    {
        ...
        else if (useOctagon)
        {
            ...
            // Convert neighbors to Point2D for oriented octagon computation
            std::vector<Point2D> neighbors2D;
            neighbors2D.reserve(neighbors.size());
            for (const auto& neighbor : neighbors)
            {
            ...
            std::array<Point2D, 8> orientedOctagon = computeOrientedOctagon(neighbors2D);
            neighbors.clear();          // Clear neighbors to avoid confusion
            allOctagons.emplace_back(); // Prepare to store the octagon points

            for (size_t j = 0; j < orientedOctagon.size(); ++j)
            {
                const auto& p = orientedOctagon[j];
                neighbors.emplace_back(p.x(), p.y());                 // Convert Point2D back to Point
                allOctagons.back()[j] = std::make_pair(p.x(), p.y()); // Store the octagon points
            }
        }
        else
        {
