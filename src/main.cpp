...
/* Create a window and its OpenGL context */
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (render.drawOctagons)
        {
            // for (const auto& octagon : render.getGraph()->allOctagons) // Iterate through each octagon
            // {

            auto&               octagon = render.getGraph()->allOctagons[2]; // Get the first octagon for demonstration
            std::vector<Vertex> vertices;                                    // Create a vector to hold the vertices of the octagon
            glm::vec3           currentColor = BLACK;                        // Color for the octagon
            vertices.reserve(8);                                             // Reserve space for the vertices

            if (0)
            {                                                                                                // Add the eight corners of the octagon
                vertices.push_back({glm::vec3{octagon[0].first, octagon[0].second, 0.f}, currentColor, {}}); // First corner
                for (size_t j = 1; j < octagon.size() / 2; ++j)
                {
                    const auto& point = octagon[j];
                    vertices.push_back({glm::vec3{point.first, point.second, 0.f}, currentColor, {}}); // Add the vertex to the octagon
                    vertices.push_back({glm::vec3{point.first, point.second, 0.f}, currentColor, {}}); // Add the vertex to the octagon
                }
                vertices.push_back({glm::vec3{octagon[0].first, octagon[0].second, 0.f}, currentColor, {}}); // Close the octagon
                GLobject drawOctagons(vertices, GL_LINES, false);                                            // Draw the octagon using the GLobject class
                drawOctagons.draw();

                vertices.clear();
                currentColor = BLUE;                                                                         // Reset the vertices vector and color for the second half of the octagon
                vertices.push_back({glm::vec3{octagon[4].first, octagon[4].second, 0.f}, currentColor, {}}); // First corner
                for (size_t j = octagon.size() / 2; j < octagon.size(); ++j)
                {
                    const auto& point = octagon[j];
                    vertices.push_back({glm::vec3{point.first, point.second, 0.f}, currentColor, {}}); // Add the vertex to the octagon
                    vertices.push_back({glm::vec3{point.first, point.second, 0.f}, currentColor, {}}); // Add the vertex to the octagon
                }
                vertices.push_back({glm::vec3{octagon[4].first, octagon[4].second, 0.f}, currentColor, {}}); // Close the octagon
                GLobject drawOctagons2(vertices, GL_LINES, false);                                           // Draw the octagon using the GLobject class
                drawOctagons2.draw();
            }
            else
            {
                vertices.push_back({glm::vec3{octagon[0].first, octagon[0].second, 0.f}, currentColor, {}}); // First corner
                for (size_t j = 1; j < octagon.size(); ++j)
                {
                    const auto& point = octagon[j];
                    vertices.push_back({glm::vec3{point.first, point.second, 0.f}, currentColor, {}}); // Add the vertex to the octagon
                    vertices.push_back({glm::vec3{point.first, point.second, 0.f}, currentColor, {}}); // Add the vertex to the octagon
