...
void Render::render2D()
{
    ...
    if (graphe.useWelzl)
        ImGui::Text("Current centralisation method: Welzl Circle");
    else if (graphe.useCentroid)
        ImGui::Text("Current centralisation method: Centroid");
    else if (graphe.useSquare)
        ImGui::Text("Current centralisation method: Square");
    else if (graphe.kNearest)
        ImGui::Text("Current centralisation method: kNearest");
    else if (graphe.useOrientedBox)
        ImGui::Text("Current centralisation method: Oriented Square");
    else if (graphe.useMean)
        ImGui::Text("Current centralisation method: Mean");
    else if (graphe.useOctagon)
        ImGui::Text("Current centralisation method: Octagon");
    else
        ImGui::Text("Current centralisation method: ERROR NOT FOUND");
    ...
    if (ImGui::Button("Switch to Square"))
    {
        ...
        graphe.useOctagon        = false; // Toggle the use of octagon calculation
    }
    ...
    if (ImGui::Button("Switch to octagon"))
    {
        graphe.nbrCentralisation = 0;     // Reset the number of centralisations
        itrCentralisation        = 0;     // Reset the centralisation counter
        graphe.useWelzl          = false; // Toggle the use of Welzl's algorithm
        ...
        graphe.useOrientedBox    = false; // Toggle the use of oriented bounding box
        graphe.useMean           = false; // Toggle the use of mean calculation
        graphe.useOctagon        = true;  // Toggle the use of octagon calculation
    }
    ...
    if (ImGui::Button("Show Octagons"))
    {
        drawOctagons = !drawOctagons; // Toggle the drawing of octagons
    }
    ...
    {
        save_energies_to_csv(graphe.energies, ASSETS_PATH + std::string{"PointExemple/pointListV0Octagon.csv"}); // Save the energies to a CSV file
